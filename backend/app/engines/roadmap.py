...
def _gemini_resolve(goal_text: str, sector: Optional[str], level: Optional[str]) -> Dict:
    ...
    prompt = (
        "PathFinderAI has GROUNDED roadmaps for these data & analytics roles only:\n"
...
def resolve_goal(goal_text: str, sector: Optional[str] = None, level: Optional[str] = None) -> Dict:
    ...
    if settings.GEMINI_API_KEY and goal_text:
        try:
            ...
            print(f"[PathFinderAI] Gemini goal-resolve failed, using local: {exc}", file=sys.stderr)
...
def build_ai_roadmap(role_title: str, field: str, level: Optional[str] = None,
    ...
    try:
        ...
        print(f"[PathFinderAI] Gemini AI-plan failed, using fallback: {exc}", file=sys.stderr)
    ...
    return {
        ...
        "data_source": "Generated by Gemini (gemini-2.5-flash) for a field outside PathFinderAI's grounded catalog.",
    }
